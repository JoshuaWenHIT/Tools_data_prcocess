...
def convert(imgdir, annpath):
    ...
    for img_id, key in enumerate(ann.keys()):
        ...
        for region in regions.values():
            ...
            print(cat)
            assert cat in ['pig', 'pig_bad',
                           'pig_cw']  # 如果你的返回的是子类名字。那么这一段就需要。assert cat in ['name1', 'name2', 'name3',...]
            if cat == 'pig':  # if cat == 'name1':
                cat_id = 1
            elif cat == 'pig_bad':
                cat_id = 2
            elif cat == 'pig_cw':
...
if __name__ == '__main__':
    img_path = '/home/ubuntu/Documents/SCpic/cw/all/val/'  # 改成自己的图片路径
    anno_path = '/home/ubuntu/Documents/SCpic/cw/all/val_via/via_region_data.json'  # 自己的标注文件的路径。注意这里不是使用的VIA导出的coco格式文件，而是单纯的json格式文件。
    result_path = '/home/ubuntu/Documents/SCpic/cw/all/val_coco/new.json'  # 输出，结果文件
